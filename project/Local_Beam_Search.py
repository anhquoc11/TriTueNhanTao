...
def solve(grid, start_i, start_j):
    ...
    actions = []
    find = False
    node_start = Node(start_i,start_j,start_state,None,None,h(start_state),0)
    queue_priority = [node_start]
    reached = [tuple(tuple(row) for row in start_state)]
    while queue_priority:
        children = []
        k = 3
        if len(queue_priority) < k: k = len(queue_priority)
        for index in range(k):
            ...
            for action in moves:
                ...
                cost = h(state_child)
                child = Node(i_child,j_child,state_child,action,parent,cost,parent.PATH_COST + 1)
                state_key = tuple(tuple(row) for row in state_child)
                if state_key not in reached: 
                    children = add_(children,child)
                    reached.append(state_key)
        if find: break
        queue_priority.clear()
        if len(children) < k: k = len(children)
