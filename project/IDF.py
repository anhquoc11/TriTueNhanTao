import random
import copy 

class Node():
    def __init__(self,i, j, state, action, parent, cost):
        self.i = i
        self.j = j
        self.STATE = state
        self.ACTION = action
        self.PARENT = parent
        self.COST = cost

def get_actions(i,j, grid):
    max_rows = len(grid)
    max_cols = len(grid[0]) if grid else 0
    result = []
    if i > 0: result.append("UP")
    if i < max_rows - 1: result.append("DOWN")
    if j > 0: result.append("LEFT")
    if j < max_cols - 1: result.append("RIGHT")
    return result

def remove_block(i, j, move, x):
    result = move.copy()
    if "UP" in result and x[i-1][j] == 2: result.remove("UP")
    if "DOWN" in result and x[i+1][j] == 2: result.remove("DOWN")
    if "LEFT" in result and x[i][j-1] == 2: result.remove("LEFT")
    if "RIGHT" in result and x[i][j+1] == 2: result.remove("RIGHT")
    return result

def next_pos(i,j,x):
    if x == "UP": return i-1,j
    if x == "DOWN": return i+1,j
    if x == "RIGHT": return i,j+1
    if x == "LEFT": return i,j-1

def print_board(board):
    for row in board:
        print(row)
    print()

def find_goal(a):
    GOAL = copy.deepcopy(a)
    for t in range(4):
        for z in range(4):
            if GOAL[t][z] != 2: GOAL[t][z] = 0
    return GOAL

def depth_limited_search(grid, start_i, start_j, depth_limit):
    GOAL = find_goal(grid)
    start_state = copy.deepcopy(grid)
    start_state[start_i][start_j] = 0
    if start_state == GOAL:
        return [start_state], []

    stack = [Node(start_i, start_j, start_state, None, None, 0)]
    visited = {(start_i, start_j, tuple(tuple(row) for row in start_state))}

    while stack:
        cur = stack.pop()
        if cur.COST > depth_limit:
            continue

        move_list = remove_block(cur.i, cur.j, get_actions(cur.i, cur.j, cur.STATE), cur.STATE)
        for action in reversed(move_list):
            state_child = copy.deepcopy(cur.STATE)
            i_child, j_child = next_pos(cur.i, cur.j, action)
            state_child[i_child][j_child] = 0
            child = Node(i_child, j_child, state_child, action, cur, cur.COST + 1)
            if child.COST > depth_limit:
                continue
            state_tuple = tuple(tuple(row) for row in state_child)
            key = (i_child, j_child, state_tuple)
            if key in visited:
                continue
            visited.add(key)
            if state_child == GOAL:
                path_states = []
                actions = []
                node = child
                while node is not None:
                    path_states.append(node.STATE)
                    if node.ACTION is not None:
                        actions.append(node.ACTION)
                    node = node.PARENT
                path_states.reverse()
                actions.reverse()
                return path_states, actions
            stack.append(child)

    return [], []


def IDF(grid, start_i, start_j):
    max_depth = len(grid) * len(grid[0]) * 2
    for depth in range(max_depth + 1):
        path_states, actions = depth_limited_search(grid, start_i, start_j, depth)
        if path_states:
            return path_states, actions
    return [], []
    


