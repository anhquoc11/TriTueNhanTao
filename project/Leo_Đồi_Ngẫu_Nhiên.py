...
def find_goal(grid):
    return [
        [0 if cell != 2 else 2 for cell in row]
        for row in grid
    ]
...
def solve(grid, start_i, start_j):
    ...
    path_states = [copy.deepcopy(current_state)]
    actions = []
    current_i = start_i
    current_j = start_j
    ...
    while True:
        ...
        for action in moves:
            ...
            if child_score < current_score:
                better_neighbors.append((child_score,action,next_i,next_j,child))
        if not better_neighbors:
            break
        (best_score,best_action,best_i,best_j,best_state) = random.choice(better_neighbors)
        current_score = best_score
        current_state = best_state
        current_i = best_i
        current_j = best_j
        actions.append(best_action)
        path_states.append(copy.deepcopy(current_state))

    return [], []


Leo_Đồi_Ngẫu_Nhiên = solve