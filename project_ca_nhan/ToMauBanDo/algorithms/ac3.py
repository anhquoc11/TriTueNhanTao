...
class MapColoringAC3:
    def __init__(self, variables, domains, neighbors, constraint):
        self.variables = list(variables)
        self.domains = {var: set(domains[var]) for var in self.variables}
    ...
    def set_domain_to_value(self, var, value, removals):
        for x in list(self.domains[var]):
            if x != value:
                self.domains[var].remove(x)
                removals.append((var, x))
    ...
    def select_unassigned_variable(self, assignment):
        ...
        return min(
            unassigned,
            key=lambda v: (len(self.domains[v]), -len(self.neighbors.get(v, [])))
        )
    ...
    def backtrack(self, assignment):
        ...
        if var is None or not self.domains[var]:
            return None
        ...
        for value in self.order_domain_values(var, assignment):
            if self.consistent(var, value, assignment):
                ...
                removals = []
                self.set_domain_to_value(var, value, removals)
                queue = deque((neighbor, var) for neighbor in self.neighbors.get(var, []))
                if self.ac3(queue, removals):
...
def create_map_coloring_ac3(path, colors=None, fixed_colors=None, forbidden_colors=None):
    if colors is None:
    ...
    return MapColoringAC3(variables, domains, neighbors, map_color_constraint)


def solve_map_coloring(path, colors=None, fixed_colors=None, forbidden_colors=None):
    csp = create_map_coloring_ac3(path, colors, fixed_colors, forbidden_colors)
    removals = []
    if not csp.ac3(removals):
        return None
...
def solve_map_coloring_steps(path, colors=None, fixed_colors=None, forbidden_colors=None):
    ...
    removals = []
    if not csp.ac3(removals):
        yield {"action": "constraint_failed", "message": "AC-3 không thể tìm thấy miền tương thích"}
        return
